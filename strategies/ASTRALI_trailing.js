...
let buyAdxOver = 30;
let buyCciUnder = -100;
let buyPrevRsiUnder = 26

//all of those need to happen for a sell
let sellAdxOver = 30;
let sellCciOver = 100;
let trailSellPercent = 5;
...
buyPrevRsiUnder = this.settings.buyPrevRsiUnder

sellAdxOver = this.settings.sellAdxOver;
sellCciOver = this.settings.sellCciOver;

trailSellPercent = this.settings.trailSellPercent;
...
this.addTulipIndicator('RSI', 'rsi', { optInTimePeriod: rsiOptIn }) ;//optInTimePeriod:(From 1 to 100000)
...
if (rsiResult > rsiResultPrev && rsiResultPrev < buyPrevRsiUnder) {
//	console.log('trix uptrending! Yeah :D');
	timeToBuy +=1;
};
...
//	console.log('trix uptrending! Yeah :D');
console.log('RSI uptrending!');
}
else {
console.log('RSI down');
};
...
//console.log('rsi previous:' + rsiResultPrev + ' and current rsi :' + rsiResult)
